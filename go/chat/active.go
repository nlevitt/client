...
func TouchFirstChatActiveQueryTime(g *globals.Context) time.Time {
	...
	if err != nil {
		g.Log.Warning("Failed to get chat active query time: %s", err)
		return now
	}
	...
	if !found {
		...
		if err != nil {
			g.Log.Warning("Failed to store chat active query time: %s", err)
		}
		return now
	}
...
func GetLastSendTime(g *globals.Context) time.Time {
	...
	if err != nil {
		g.Log.Warning("Failed to get chat active last send time: %s", err)
		return zeroTime
	}
